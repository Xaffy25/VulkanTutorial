...
bool Application::checkDeviceExtensionSupport(VkPhysicalDevice device)
{
	...
	//We put all extensions onto a set
	std::set<std::string> requiredExtensions(deviceExtensions.begin(), deviceExtensions.end());
	//Iterating trought extensions
	for (const auto& extension : availableExtensions)
	{
		//"ticking off" supported extensions
		requiredExtensions.erase(extension.extensionName);
	}
	//if all extensions are "ticked off" return true
...
void Application::createLogicalDevice()
{
	...
	for (uint32_t queueFamily : uniqueQueueFamilies)
	{
		...
		queueCreateInfo.pQueuePriorities = &queuePriority; //Not needed but still good practice
...
VkShaderModule Application::createShaderModule(const std::vector<char>& code)
{
	...
	{
		throw std::runtime_error("failed t ocreate shader module!");
	}
...
void Application::createGraphicsPipeline()
{
	...
	rasterizer.polygonMode = VK_POLYGON_MODE_FILL; // CAN CHANGE TO VK_POLYGON_MODE_LINE
	...
	//Depth and stencil
	//We dont buffers for them configured so we skip this part for now
...
void Application::createSyncObjects()
{
	...
	fenceInfo.flags = VK_FENCE_CREATE_SIGNALED_BIT; // First call for wait passes without waiting
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
		...
		{
			throw std::runtime_error("Failed to create synchronization objects fo a frame!");
		}
	}
}
...
void Application::createDescriptorSetLayout()
{
	...
	//We are using this vbo only in Vertex
...
/*
VkSemaphore is used for GPU
VkFence is used for CPU
*/
...
void Application::cleanup()
{
	...
	vkDestroySurfaceKHR(instance, surface, nullptr); //Needs to be done before instance
